...
// branch only (matches the checkpoint, IMPLEMENTATION_PLAN.md S1.9, so theta_out is always 0).
// Deliberately NOT part of the public library: production rotation is Phase 8's SarlAdapter job
// (IMPLEMENTATION_PLAN.md S4.1). This exists only so the round-trip test can turn the builder's
...
// it (IMPLEMENTATION_PLAN.md S4.1.2). Fixture generated by test/generate_reference_fixture.py
