...
## 6. Post-fix re-run integrity (reconciliation pass)

The 142-episode re-run against the §1.1/§1.3 fixes did not go smoothly the first time: an
environment reset silently killed the first attempt at episode ~62/142, mid-`noise_sweep`,
with no error surfaced until a background-task status check returned "no task found." Before
trusting the final `episodes.csv`/`interventions.csv` as a single clean pass, this was checked
explicitly rather than assumed - a partial run silently stitched onto its own restart would
corrupt exactly the numbers this document's §2 already promised to re-derive honestly.

**What was actually done, for the record**: the partial-run artifacts (`episodes.csv`,
`interventions.csv`, and the per-episode `logs/` directory, all showing ~62 completed episodes)
were deleted entirely - not appended to - before relaunching `run_matrix.py --phase all` a
second time from a directory containing no prior results. That second invocation ran end to end,
unattended, from `[1/142]` to `[142/142]`, confirmed via the task's own sequential progress log
with no gaps or restarts in between.

**Verified after the fact, three independent ways, not just trusted from the run log**:
1. `episodes.csv` has exactly 142 rows, 142 unique `episode_id` values, 0 duplicated rows, 0
   duplicated IDs.
2. The full set of 142 `episode_id` values in `episodes.csv` was diffed against the set
   independently generated by calling `scenarios.py`'s own four episode-iterator functions
   directly (not read back from any file) - **zero missing, zero unexpected**. This rules out
   both a stitching contamination (which would show duplicates or an inflated count) and a
   silent truncation (which would show missing IDs).
3. The per-episode `results/logs/<episode_id>/` directories on disk (142 of them, one written by
   each episode's own subprocess, independent of anything `run_matrix.py`'s CSV writer does) were
   diffed against the CSV's own `episode_id` list - **exact match**, confirming the log evidence
   and the CSV evidence agree independently.

**Conclusion: the corrected results are a single clean 142-episode pass. No re-run was
necessary.**
...
describing the pre-fix world even after the numbers it cites have moved. This section is that
check, done explicitly rather than assumed, with each claim labeled by outcome.

**Held, re-verified against the corrected CSVs, left unedited (beyond confirming the cited
number matches)**:
- Efficiency: `policy_raw`/`policy_supervised` remain slower and take longer paths than
  `baseline_mppi` in every family - confirmed against the corrected per-episode data.
- The noise sweep's *rate-normalized* saturation mechanism (interventions per simulated second
  jumps 0.0&rarr;0.1, then stays flat through 0.5) - re-derived from the corrected CSV and
  unchanged in shape.
- The OOD-reachability fixes (§1.1) - re-confirmed firing under the corrected frame:
  `CROWD_SIZE` x433, `RELATIVE_SPEED` x15, `INFERENCE_TIMEOUT` x1, `COMMAND_LIMIT` still 0 and
  still mathematically unreachable (a proven zero, not a gap).
- Sample-size/contamination integrity (§2, §3 above): re-run against the corrected 142-episode
  file, same clean result.

**Reversed - rewritten to state the corrected finding, with the reversal itself stated
explicitly, not silently swapped**:
- **The headline reactive-pedestrian collision-rate finding.** Pre-fix: `policy_supervised`
  25-50% vs. `baseline_mppi`/`policy_raw` 12% (the supervisor looked worse than doing nothing
...
- **The detection-margin timing breakdown.** Pre-fix: 6 reactive-mode `policy_supervised`
  ...
  caught within a quarter second of contact (0.22 s, 0.07 s) - re-derived directly from
  `interventions.csv` in this reconciliation pass, not assumed to still hold. Rewritten to
  describe a narrow, last-instant margin limit, not a broad gap - the original explanation's
  *category* (a training-distribution-inherited margin, not a logic error) still applies to the
  smaller residual, but the *severity* claim ("dominant pattern," "4 of 6") does not survive and
  was removed.
- **The noise-sweep "cliff" location.** Pre-fix: described as "a sharp, reproducible failure
  cliff between `dropout_prob` 0.1 and 0.2" (outcome-level success collapsing 7/8&rarr;0/8 at
  that specific boundary). Post-fix: the corrected outcome distribution shows a more gradual
  decline starting a point earlier (success 4/8 at 0.0, 2/8 at 0.1, 0/8 at 0.2) - found by
  re-deriving the per-`dropout_prob` outcome breakdown directly from the corrected CSV rather
...
- **The central thesis.** Pre-fix framing ("the supervisor's reliability tracks how
  ...
  static hazard; closes a real gap against a dynamic one, with a narrow residual; saturates
  under degraded input) still holds and is arguably a *more* defensible three-part story now
  that the dynamic case is a clear win rather than an uncomfortable loss - but every place that
  used the old framing to imply the supervisor was merely "not making things worse" rather than
  "measurably closing a real safety gap" was rewritten to say the stronger, corrected thing.

**Removed - the claim existed only to explain a result that no longer exists**:
- The original run's explanation for *why* the supervised policy collided more than the raw
  policy (attributed to `PROXIMITY`'s detection margin not keeping pace with reactive humans,
  applied as the explanation for 4 of 6 collisions happening seconds after the last
  intervention) is removed as a standalone claim - it was explaining a collision pattern
  (multiple, separately-timed collisions with a supervisor that was nominally engaged but
  under-protective) that the corrected data does not reproduce. What remains, correctly scoped
...
not perception/crowd effects"). With no real humans in `WorldState.humans`, the §1.3 fix's
per-human offset correction has nothing to act on, and the supervisor's forward-sim/costmap
